...
const router = Router();
router.post('/generate-post', authMiddleware, validateInput, async (req, res) => {
    try {
        ...
        try {
            ...
            parsedPosts = Array.from({ length: expectedCount }, (_, i) => createAuthenticFallback(prompt, i, expectedCount, PostType));
        }
        ...
        res.status(500).json({
            ...
            details: env.NODE_ENV === 'development' ? error.message : 'Internal server error'
        });
    }
});
router.get("/posts", authMiddleware, async (req, res) => {
    try {
        ...
        const threads = Array.from(grouped.values()).map((group) => group.sort((a, b) => a.postNumber - b.postNumber));
...
router.delete('/posts/thread/:threadId', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
...
router.post('/posts/save', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const threadId = PostType !== 'single' ? crypto.randomUUID() : null;
        const saved = await Promise.all(posts.map(async (post, index) => {
